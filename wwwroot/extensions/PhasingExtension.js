...
class PhasingPanel extends Autodesk.Viewing.UI.DockingPanel {
  ...
  updateObjects(result) {
    console.log(phasing_config.mapTaksNProps);
    console.log(result);  
    let currentTaskId = phasing_config.mapTaksNProps[result.externalId];
    if (!!currentTaskId) {
  ...
  handleElementsColor() {
    ...
    if (overrideCheckbox.checked) {
      ...
      for (let index = 0; index < this.gantt.tasks.length; index++) {
        ...
        console.log(phasing_config);
        console.log(currentdbIds);
  ...
  lineToObject(line, inputHeadersLine, dictionary) {
    ...
    Object.values(phasing_config.requiredProps).forEach(requiredProp => {
      ...
      newObject.id = String(dictionary[newObject.id]);
...
class PhasingExtension extends Autodesk.Viewing.Extension {
  ...
  async update() {
    ...
    // console.log("These are found dbids" + dbids)
  }
...
async function mappingUniqueIdtoDbid(viewer) {
  try {
      let idDict = await new Promise((resolve, reject) => {
          viewer.model.getExternalIdMapping(data => {
              if (data) {
                  resolve(data);
              } else {
                  reject(new Error('No data received from getExternalIdMapping'));
              }
          });
      });
      return idDict;
